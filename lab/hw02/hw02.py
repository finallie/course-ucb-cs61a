...
def waves(n):
    ...
    def helper(n, count, prev):
        ...
        if rest == 0:
            return count == 0
        a, b = cmp(prev, curr), cmp(curr, next)
        if a == 0 or b == 0 or a == b:
            return helper(rest, count, prev)
        return helper(rest, count + b, curr)
...
def count_coins(total):
    ...
    def helper(total, max_coin):
        ...
        if total == 0:
            return 1
        return helper(total, next_smaller_coin(max_coin)) + (
            helper(total - max_coin, max_coin) if total >= max_coin else 0
        )
