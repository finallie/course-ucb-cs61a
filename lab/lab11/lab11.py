...
def floor_div(expr):
    ...
    """
    # BEGIN SOLUTION Q2
    value, expr = expr.first, expr.rest
...
def is_scheme_false(v):
    return v is False


def is_scheme_true(v):
    return not is_scheme_false(v)
...
def eval_and(operands):
    ...
    # BEGIN SOLUTION Q3
    value = True
    while is_scheme_true(value) and operands is not nil:
...
def eval_define(expr):
    ...
    """
    # BEGIN SOLUTION Q4
    symbol, value = expr.first, calc_eval(expr.rest.first)
