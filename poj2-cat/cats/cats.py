...
def about(subject):
    ...
    def select(p):
        for w in split(lower(remove_punctuation(p))):
            if w in s:
                return True
        return False
...
def accuracy(typed, source):
    ...
    # BEGIN PROBLEM 3
    "*** YOUR CODE HERE ***"
    l1, l2 = len(typed_words), len(source_words)
    if l1 == 0 and l2 == 0:
        return 100.0
    if l1 == 0 or l2 == 0:
        return 0.0
...
def memo_diff(diff_function):
    ...
    def memoized(typed, source, limit):
        ...
        if (typed, source) in cache:
            value, max_limit = cache[(typed, source)]
            if limit <= max_limit:
                return value
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    w = min(
        word_list,
        key=lambda x: negative_infinity
        if x == typed_word
        else diff_function(typed_word, x, limit),
    )
    return typed_word if diff_function(typed_word, w, limit) > limit else w
    # END PROBLEM 5
...
def fastest_words(match):
    ...
    for i in word_indices:
        min_player = min(player_indices, key=lambda x: time(match, x, i))
        lst[min_player].append(get_word(match, i))
    return lst
    # END PROBLEM 10
