...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        place = self.place
        i = 0
        while place is not None and i < self.lower_bound:
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def action(self, gamestate):
        ...
        while place:
            if place.ant:
                if place.ant not in QueenAnt.doubled_ants:
                    place.ant.double()
                    QueenAnt.doubled_ants.add(place.ant)
                if (
                    isinstance(place.ant, ContainerAnt)
                    and place.ant.ant_contained
                    and place.ant.ant_contained not in QueenAnt.doubled_ants
                ):
                    place.ant.ant_contained.double()
                    QueenAnt.doubled_ants.add(place.ant.ant_contained)
            place = place.exit
        # END Problem 12
...
class SlowThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        ...
        def action(gamestate):
            nonlocal turn
            if turn <= 0 or gamestate.time % 2 == 0:
...
class Bee(Insect):
    ...
    def action(self, gamestate):
        ...
        if self.scared and self.scare_turn > 0:
            self.scare_turn -= 1
